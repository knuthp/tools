...
while true; do
  ...
  uv run python/fetch_ais_history_to_parquet.py --year "$year" --week "$week"

  hf buckets cp "./ais_pos_${year}_W${WEEK_PADDED}.parquet" \
    hf://buckets/knuthp/ais

  rm "./ais_pos_${year}_W${WEEK_PADDED}.parquet"
